        ...
        for i in range(0,len(lstFuncDel)):
            ...
            if(len(arrItem)>=2):
                ...
                if '-' in arrItem[0] and arrItem[1].startswith('FunctionDecl'):
                    ...
                    strPseudoCodeItem=arrPseudos[beginLine-1]
                    strBeginLbl = '{}\t{}\t{}'.format(arrItem[1],beginLine, strPseudoCodeItem)
                    lstNewLabel.append(strBeginLbl)
                else:
                    beginLine = int(arrItem[0].split('-')[0]) - distanceOfCPPFile
                    endLine = int(arrItem[0].split('-')[1]) - distanceOfCPPFile
                    ...
                    strPseudoCodeItem = arrPseudos[endLine - 1]
                    strEndLbl = '{}\t{}\t{}'.format(arrItem[1], endLine, strPseudoCodeItem)

                    lstNewLabel.append(strEndLbl)
        dictLabelFuncDecl[key]=lstNewLabel
        strNewLabel='\n'.join(lstNewLabel)
        strWriteToFile='\n'.join([key,strNewLabel,'\n\n\n'])
        f1 = open(fpCSVData, 'a')
...
def prepareTrainingDataAndLabelForStatements(fpPseudoCodeData, fpFuncDeclData, fpCodeData, fpTxtData):
    f1=open(fpPseudoCodeData,'r')
    arrPseudos=f1.read().split('\n')
    f1.close()
    dictPseudoCode = {}
        ...
        for i in range(0,len(lstFuncDel)):
            ...
            if(len(arrItem)>=2):
                ...
                if '-' in arrItem[0] and arrItem[1].startswith('FunctionDecl'):
                    ...
                    strBeginLbl ='{}\t{}'.format(beginLine, arrItem[1])
                    lstNewLabel.append(strBeginLbl)
        dictLabelFuncDecl[key]=lstNewLabel
