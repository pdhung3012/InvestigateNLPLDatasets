...
fopInputStep2CodeReplaceDict=fopRoot+'step2_code_replaceDict/'
fopInputStep3V2=fopRoot+'step3_v2/'
fopFasttextEmb=fopRoot+'embeddingModels/fasttext-cbow/'
fopInputStep6Incons=fopRoot+'step6_Incons/fasttext-RF_win1/'
...
for i in range(0,len(arrSource)):
    ...
    fpItemCode=fopInputStep2CodeReplaceDict+arrTab2[2]+'/'+arrTab2[1]+'_code.cpp'
    lineNumber=int( arrTab2[3])-1
    ...
    lstTempCor=[strInputTextCorrect]
    lstTempInc=[strInputTextIncorrect]
    for j in range(lineNumber-contextSize,lineNumber+contextSize+1):
        if j!=lineNumber:
            lstTempCor.append(arrTarget[j])
            lstTempInc.append(arrTarget[j])
            # strInputTextCorrect=strInputTextCorrect+strEndLine+arrTarget[j]
            # strInputTextIncorrect = strInputTextIncorrect + strEndLine + arrTarget[j]
    strInputTextCorrect = strEndLine.join(lstTempCor)
    strInputTextIncorrect = strEndLine.join(lstTempInc)
    print('{}'.format(i))
    ...
    lstEmbInput.append('{}\t{}\t{}'.format(arrTab2[0],1,strVectorCorrect))
    lstEmbInput.append('{}\t{}\t{}'.format(arrTab2[0], 0, strVectorIncorrect))
