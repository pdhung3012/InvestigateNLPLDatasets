...
lstFpTextPOSNLTK=glob.glob(fopPOSNLTK+'*.text.txt')
lstFpTextPOSStanford=glob.glob(fopPOSStanford+'*.text.txt')
...
for i in range(0,len(lstFpTextPOSNLTK)):
    ...
    for j in range(0,len(arrItemText)):
        ...
        if linePOS!='{}':
            dictNLTKAlreadyAppeared[lineText]=nameItem
...
for i in range(0,len(lstFpTextPOSStanford)):
    ...
    for j in range(0,len(arrItemText)):
        ...
        lineText=arrItemText[j]
        if linePOS!='{}':
            dictStanfordAlreadyAppeared[lineText]=nameItem
...
numAlreadyNLTK=0
numAlreadyStanford=0
dictAppendToNLTK={}
dictAppendToStanford={}
for i in range(0,len(arrSources)):
    itemSource=arrSources[i]
    if itemSource in dictNLTKAlreadyAppeared.keys():
        numAlreadyNLTK=numAlreadyNLTK+1
    else:
        dictAppendToNLTK[itemSource]=1
    if itemSource in dictStanfordAlreadyAppeared.keys():
        numAlreadyStanford=numAlreadyStanford+1
    else:
        dictAppendToStanford[itemSource]=1

print('percentage NLTK {}/{} {}'.format(numAlreadyNLTK,len(arrSources),len(dictAppendToNLTK.keys())))
print('percentage Stanford {}/{} {}'.format(numAlreadyStanford,len(arrSources),len(dictAppendToStanford.keys())))
...
lstText=list(dictAppendToStanford.keys())
lstItemText=[]
lstItemPOS1=[]
indexItem=0
for i in range(0,len(lstText)):
    try:
        ...
        traceback.print_exc()
    if indexItem==len(lstText)-1 or indexItem%100==0:
        f1 = open(fpAppendStanfordText, 'a+')
        f1.write('\n'.join(lstItemText) + '\n')
        f1.close()
        f1 = open(fpAppendStanfordPOS, 'a+')
        f1.write('\n'.join(map(str,lstItemPOS1)) + '\n')
        f1.close()
        lstItemText=[]
        lstItemPOS1=[]
        print('Stanford complete for index {}'.format(indexItem))


lstText=list(dictAppendToNLTK.keys())
lstItemText=[]
lstItemPOS2=[]
indexItem=0
for i in range(0,len(lstText)):
    try:
        ...
        traceback.print_exc()
    if indexItem==len(lstText)-1 or indexItem%100==0:
        f1 = open(fpAppendNLTKText, 'a+')
        f1.write('\n'.join(lstItemText) + '\n')
        f1.close()
        f1 = open(fpAppendNLTKPOS, 'a+')
        f1.write('\n'.join(map(str,lstItemPOS2)) + '\n')
        f1.close()
        lstItemText=[]
        lstItemPOS2=[]
        print('NLTK complete for index {}'.format(indexItem))
