...
def generateGraphForMixCodeRemoveNLNode(jsonObject,arrCodes,strRootProgramId,strRootLabel,isInNLNodes,graph,dictGraphIndexContext,dictAcceptableIdsForVersions):
    ...
    if 'children' in jsonObject.keys():
        ...
        for i in range(0, len(lstChildren)):
            if isInNLNodes:
                continue
            strChildLabel = generateGraphForMixCodeRemoveNLNode(lstChildren[i],arrCodes,strRootProgramId,strRootLabel,isInNLNodes,graph,dictGraphIndexContext,dictAcceptableIdsForVersions)
            if strLabel != strChildLabel:
                graph.add_edge(strLabel, strChildLabel, color='black')
            if not isInNLNodes:
...
fopOutputAugmentation=fopRoot+'step4_removeAllNLNodes/'
...
for i in range(0,len(lstFpJsonFiles)):
    ...
    try:
        ...
        for j in range(0,len(lstFpVersionLabel)):
            ...
            generateGraphForMixCodeRemoveNLNode(jsonMixClone, arrFinalCodes, strRootProgramId, strRootLabel, isInNLNode, graphAll,dictGraphIndexContext,dictAcceptableIdsForVersions)
