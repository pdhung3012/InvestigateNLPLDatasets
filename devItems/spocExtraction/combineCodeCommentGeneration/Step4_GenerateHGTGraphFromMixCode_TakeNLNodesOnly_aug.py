...
def getLabelStringOfNLNodeJson(jsonObject):
...
def generateGraphForMixCodeTakeOnlyLeafNLNode(jsonObject, arrCodes, strRootProgramId, strRootLabel, isInNLNodes, graph, dictGraphIndexContext, dictAcceptableIdsForVersions):
    strValue='EMPTY'
    isAbstract = False
    strType='ASTNode'
    strLabelInClassification=''
    ...
    if isInNLNodes:
        ...
        lstLeafs = []
        getLeafChildren(jsonObject, lstLeafs)
        for leaf in lstLeafs:
            strChildLabel = getLabelStringOfNLNodeJson(leaf)
            graph.add_node(strChildLabel, color='red')
            for numContext in dictGraphIndexContext.keys():
                graphId = dictGraphIndexContext[numContext]
                graphId.add_node(strChildLabel, color='red')
            if strLabel != strChildLabel:
                graph.add_edge(strLabel, strChildLabel, color='black')
                for numContext in dictGraphIndexContext.keys():
                    graphId = dictGraphIndexContext[numContext]
                    graphId.add_edge(strLabel,strChildLabel, color='red')
    ...
    if 'children' in jsonObject.keys():
        ...
        for i in range(0, len(lstChildren)):
            if isInNLNodes:
                continue
            strChildLabel = generateGraphForMixCodeTakeOnlyLeafNLNode(lstChildren[i], arrCodes, strRootProgramId, strRootLabel, isInNLNodes, graph, dictGraphIndexContext, dictAcceptableIdsForVersions)
            if strLabel != strChildLabel:
                graph.add_edge(strLabel, strChildLabel, color='black')
            if not isInNLNodes:
...
def getLeafChildren(jsonObject,lstLeaf):
    if not 'children' in jsonObject.keys():
        lstLeaf.append(jsonObject)
    else:
        lstChildren = jsonObject['children']
        for i in range(0, len(lstChildren)):
            getLeafChildren(lstChildren[i],lstLeaf)
...
fopOutputAugmentation=fopRoot+'step4_takeLeafNLNodes_augmentation/'
...
for i in range(0,len(lstFpJsonFiles)):
    ...
    fopItemSeparateGraph=fopItemProgram.replace(fopMixVersion,fopOutputAugmentation)
    ...
    try:
        ...
        lstFopItemGraphFolders=glob.glob(fopItemSeparateGraph+'v_*_graphs/')
        ...
        createDirIfNotExist(fopItemSeparateGraph)
        fpCodeLogOutput=fopItemSeparateGraph+'a_logPrint.txt'
        ...
        for j in range(0,len(lstFpVersionLabel)):
            ...
            fopItemVersionGraph = fopItemSeparateGraph + fnVersionName+ '_graphs/'
            ...
            generateGraphForMixCodeTakeOnlyLeafNLNode(jsonMixClone, arrFinalCodes, strRootProgramId, strRootLabel, isInNLNode, graphAll, dictGraphIndexContext, dictAcceptableIdsForVersions)
            augmentLeafNodes(graphAll)
            ...
            if countNumMixCode<=20:
                graphAll.layout(prog='dot')
                graphAll.draw(fpItemGraphPng)
            countNumMixCode = countNumMixCode + 1

        sys.stdout.close()
        sys.stdout = sys.__stdout__
        print('end {}/{} {}'.format(i,len(lstFpJsonFiles),fpItemAST))
        print('countNumMixCode {}'.format(countNumMixCode))
        # if i==0:
        #     break
        if countNumMixCode >= 23000:
            break
    except:
        traceback.print_exc()
